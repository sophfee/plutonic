...
if CLIENT and (not ConVarExists("longsword_debug") or not ConVarExists("longsword_centered")) then
...
-- Plays a sound if not in first person perspective
if SERVER then
...
function SWEP:GetOffset()
	...
	if self:GetIronsights() then
		local offset_pos =  LerpVector(self:GetIronsightsRecoil(),Vector(),VectorRand(-0.2,0.2))
...
local c_compensate = Angle(0,0,0)

SWEP.SwayRightMultiplier = 1
SWEP.SwayUpMultiplier = 1
...
function SWEP:GetViewModelPosition( pos, ang )
	...
	local compensate = Angle()
	compensate.p = math.AngleDifference(ang.p,lastAng.p)
	compensate.y =  math.AngleDifference(ang.y,lastAng.y)
	compensate.r = math.AngleDifference(ang.r, lastAng.r)
	c_compensate = LerpAngle(FrameTime()*10,c_compensate,compensate)
...
function SWEP:DrawHUD()
	...
	if (impulse_DevHud or debugMode:GetBool()) then
		...
		surface.DrawText("c_move: " .. self.IronsightsLerp)
