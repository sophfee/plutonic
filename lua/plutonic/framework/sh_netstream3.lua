...
if (SERVER) then
	...
	function netstream.Start(player, name, ...)
		...
		local dataTable = {...};
		local encodedData = util.Compress(pon.encode(dataTable)); -- EDITS HERE FOR NS3: COMPRESS DATA

		if (encodedData and #encodedData > 0 and bShouldSend) then
			net.Start("NetStreamDS");
				net.WriteString(name);
				-- EDITS HERE FOR NS3:
				-- DO NOT USE 32 BITS TO WRITE LENGTH OF DATA
				-- SUBTRACT (STRING LEN - MESSAGE LEN) / 8
				-- or use net.BytesLeft();
	...
	net.Receive("NetStreamDS", function(length, player)
		...
		local NS_DS_LENGTH = (length / 8) - #NS_DS_NAME; -- remaining bits.
		local NS_DS_DATA = util.Decompress(net.ReadData(NS_DS_LENGTH));
	...
	function netstream.Start(name, ...)
		local dataTable = {...};
		local encodedData = util.Compress(pon.encode(dataTable));
	...
	net.Receive("NetStreamDS", function(length)
		local NS_DS_NAME = net.ReadString();
		local NS_DS_LENGTH = (length / 8) - #NS_DS_NAME;
		local NS_DS_DATA = util.Decompress(net.ReadData(NS_DS_LENGTH));
